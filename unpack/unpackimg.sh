...
if [ -d $img/split_img -o -d $img/ramdisk ]; then
  if [ -d $img/ramdisk ] && [ "$(stat $statarg $img/ramdisk | head -n 1)" = "root" -o ! "$(find $img/ramdisk 2>&1 | cpio -o >/dev/null 2>&1; echo $?)" -eq "0" ]; then
    rmsumsg=" (as root)";
  fi;
...
echo " ";
mkdir -p "$file"f/split_img "$file"f/ramdisk;
